...
class PerformanceApp(QtWidgets.QMainWindow):
	...
	def _on_stop(self) -> None:
		...
		# Check if AAOS is still running BEFORE marking current OS as stopped
		# This is needed to restart AAOS tail process if Linux is being stopped
		aaos_still_running = False
		if hasattr(self, 'os_running_states') and os_sel != "AAOS":
			aaos_still_running = self.os_running_states.get("AAOS", False)
		
		# Store AAOS serial if needed for restart
		aaos_serial = None
		if aaos_still_running:
			try:
				devs = _adb_list_devices()
				if devs:
					aaos_serial = devs[0][0]
			except Exception:
				pass
		...
		# If AAOS is still running, restart its ADB tail process
		# This ensures the AAOS live log continues to update after stopping Linux
		if aaos_still_running and aaos_serial is not None:
			print("[DEBUG] AAOS still running, restarting ADB tail process for live log updates")
			self._start_adb_tail(aaos_serial)
