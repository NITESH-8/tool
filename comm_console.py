			...
			if self._serial is not None and self._serial.in_waiting:
				...
				if data:
					try:
						...
						text = str(data)
					# If capturing, divert to capture buffer and do not echo to UI
					if getattr(self, '_capture_active', False):
						self._capture_buffer += text
						end_token = self._capture_end_token or ""
						if end_token and end_token in self._capture_buffer:
							parts = self._capture_buffer.split(end_token, 1)
							payload = parts[0]
							cb = self._capture_callback
							# Reset capture state before invoking callback
							self._capture_active = False
							self._capture_buffer = ""
							self._capture_end_token = None
							self._capture_callback = None
						...
						# Normal path: append to per-port log buffer and UI
						port = self.uart_port_combo.currentText()
						self._port_logs[port] = self._port_logs.get(port, "") + text
						if hasattr(self, 'log'):
							self.log.moveCursor(QtGui.QTextCursor.End)
